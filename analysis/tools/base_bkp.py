...
def edgeCleanEdged(parity,edges): #Cleans up the edges and identifies whether it is a rising or falling edge
    ...
    while (i<edges.size):
            ...
            out_parity.append(int(parity[i]-parity[i-1]))             
...
def drawTIE(TIE1,save_name="",bns=100,cutoff=0,MULT_FACT=1,fit=False,NN=100000):
    # if TIE1.size < 500:
    #     print ("TIE Size too small :",TIE1.size)
    #     pass


    fig,ax = plt.subplots(figsize=(8,6))

    Y,bins,_=ax.hist(TIE1*MULT_FACT,bins=bns,label="TIE Hist")
    # _=ax.hist(TIE2*MULT_FACT,bins=bns,label="odd edge")
    # _=ax.hist(TIE,bins=bns,label="All TIE")
    ...
    if (fit==False):
        ...
        plt.plot(xx, gauss_function(xx, *popt),
                label=' fit '
                ,linestyle='dashed',linewidth=5)

        textstr = '$\sigma$ =   '+str(np.abs(round(popt[2]*10**3,3)))+' ps'
        textstr += '\n$\mu$ =  '+str(round(popt[1]*10**3,3))+' ps'
        ...
        ax.set_xlim(popt[1]-3*np.abs(round(popt[2],3)),popt[1]+3*np.abs(round(popt[2],3)))            


    # m, s = norm.fit(TIE1*MULT_FACT) # get mean and standard deviation  
    # pdf_g = norm.pdf(bin_mid, m, s) # now get theoretical values in our interval  
    # ax.plot(bin_mid, pdf_g, label="Norm") # plot it
    # print (f"Mu = {m}, Std.Dev = {s} ")



    applyFormatting(ax,fig,NN)

    ax.set_xlabel("TIE (ns)",fontsize=15)
...
def FFT(x,y,ylim=(0,1),xlim=(0,1600),save_name=None,MEASURE_PERIOD=1,MULT_FACT=1,NN=100000,disp=0):
    ...
    #Select Window over the entire length... for sine waves
    ...
    ax.set_ylabel("Time(ps)",fontsize=15)
...
def DataAq(cycles=2,ser="DDMTD",data_save_folder="./data",save_name="test",compiles=0):
    cmd = f"../RPi_Side/runAtNex.sh data_acq.exe {compiles} {cycles} {ser}"
    runBash(cmd,show=1)
    cmd = f'scp {ser}:Flash_Firmware/ddmtd1.txt   {data_save_folder+save_name+"_ddmtd1.txt"}'
    runBash(cmd)
    cmd = f'scp {ser}:Flash_Firmware/ddmtd2.txt   {data_save_folder+save_name+"_ddmtd2.txt"}'
    runBash(cmd)
